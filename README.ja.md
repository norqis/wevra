...
| `dogfooding` | 対象 workspace の運用手順を読み、実際のフローを試し、直せる問題を修正しながら、残課題が落ち着くまで検証を繰り返します。 |

そのほか、

- 質問への回答と作業の再開
- 使用制限などで中断した AI 実行の再開と AI 切り替え
- 1つの依頼から依存関係つきの複数ジョブ案を作るジョブ分割 preview
- 依存関係と独立した workspace にもとづく安全な実行制御
- 危険なエージェント実行に対する承認ゲート
- Web dashboard 上での運用
- 進行中のジョブに対する追加指示
- 運用手順にもとづいて実際の流れを検証する dogfooding
...
- 単一ジョブを作るか、依存関係つきの複数ジョブ案を preview してから投入する
- 実行モード、承認方式、使う AI、作業ディレクトリ、必要なら運用手順パスと依存ジョブを設定する
- 進行状況、タスク、レビュー、エージェントの実行ログ、結果をリアルタイムで見る
- 作業が止まったときに質問へ回答し、エージェント実行を許可または拒否し、中断した AI 実行を再開または修復する
- 依存先が失敗または取り下げになった後続ジョブをどう扱うか選ぶ
...
3. 必要なら、依存関係つきの複数ジョブ案を preview してから投入します。
4. Wevra が mode に応じて必要な作業へ分解します。
5. ジョブは基本直列で進み、workspace が重ならない独立ジョブだけ明示的に並列実行できます。
6. 確認が必要になったら質問して止まります。
7. 外部 runtime の実行に承認が必要な場合は、`エージェント` タブで許可または拒否されるまで停止します。
8. 依存先が失敗または取り下げになったジョブはそのまま実行されず、概要から依存を無視して進めるか、取り下げるかを選べます。
9. `implementation` mode では、実装後に既存テストと最終レビューを行います。
10. `dogfooding` mode では、運用手順を読み、実際の流れを試し、修正と再確認を繰り返します。
11. 最終レビューが通ったときだけ完了します。
